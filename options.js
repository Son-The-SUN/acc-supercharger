// options.js - Save/load APS credentials and manage companies cache
...
function addAccountIdRow(value = "") {
  ...
  removeBtn.addEventListener("click", () => {
    ...
    if (rows.length > 1) {
      ...
      input.value = ""; // keep at least one row, just clear it
    }
  });
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  document.getElementById("save").addEventListener("click", () => {
    ...
    chrome.storage.local.set(
      ...
      () => {
        // Remove the old single-value key if it exists
        chrome.storage.local.remove("apsAccountId");
        statusEl.textContent = `Saved! (${accountIds.length} hub${accountIds.length > 1 ? "s" : ""}) Reload the ACC page to apply.`;
  ...
  buildCacheBtn.addEventListener("click", () => {
    ...
    cacheProgressEl.textContent = "Starting cache build...";
    ...
    chrome.runtime.sendMessage(
      ...
      (response) => {
        ...
        cacheProgressEl.textContent = `Cache built successfully with ${response.count} companies.`;
        loadCacheStatus();
      }
    );
  });
    ...
    chrome.storage.local.get(
      ...
      (data) => {
        if (data.companiesCacheTimestamp) {
          ...
          cacheInfoEl.textContent = `Last built: ${date.toLocaleString()} (${hoursAgo}h ago) — ${companyCount} companies, ${projectCount} projects${stale ? " (stale)" : ""}`;
        } else {
